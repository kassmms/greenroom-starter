...
export const ticketSales = sqliteTable("ticket_sales", {
  ...
  /**
   * Optional tier breakdown from the ticketing integration.
   * When present, lets the settle page show per-tier verification
   * instead of just the aggregate total.
   *
   * Schema: [{ tier: string, qty: number, price: number }]
   *
   * Null for shows where the integration only provides totals.
   * The aggregate fields (qty, gross, fees) are always the source
   * of truth for math — breakdown_json is display / audit only.
   */
...
/** One price tier from ticket_sales.breakdown_json */
export type TicketTier = {
  tier: string;   // e.g. "GA Presale", "GA Door", "VIP"
  qty: number;
  price: number;  // face value per ticket (before fees)
};
